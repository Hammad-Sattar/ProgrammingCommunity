...
const ExpertLeaderboardResultScreen = () => {
  ...
  const renderTeamItem = ({item, index}) => {
    const toggleQualification = async () => {
      const updatedStatus = !item.isQualified;

      try {
        const response = await fetch(
          `${Config.BASE_URL}/api/RoundResult/UpdateQualification`,
          {
            method: 'PUT',
            headers: {
        ...
        setLeaderboardData(prev =>
          prev.map(team =>
            team.teamId === item.teamId
              ? {...team, isQualified: updatedStatus}
              : team,
          ),
        );
      } catch (error) {
    ...
    return (
      <View style={styles.row}>
        ...
        <TouchableOpacity onPress={toggleQualification}>
...
export default ExpertLeaderboardResultScreen;
